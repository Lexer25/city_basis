...
return array(
		...
		'stateExport'=>'Сохранить в файл',
		'isOnLine'=>'На связи',
		'isWp'=>'WP',
		'isTest'=>'TEST',
		'DOORSTATE'=>'Точка<br>прохода',
		'DOORSTATE_MODE'=>'Режим<br>работы',
		...
		'BASE_COUNT'	=>	'Карт<br>по базе данных',
		'DEVICE_COUNT'	=>	'Карт<br>в точке прохода',
		'DEVICE_IsActive'	=>	'Вкл',
		'SCUD_MODE'	=>	'SCUD<br>MODE',
		'Comparekey'	=>	'Найти и удалить лишние карты',
		'checkStatus'	=>	'Записать текущее состояние в БД',
		...
		'card'	=>	'Номер идентификатора',
		'card_type'	=>	'Тип идентификатора',
		...
		'hint_-1'=>'Нет',
		'hint0'=>'Отказ правильный',
		'hint1'=>'Требуется анализ даты события и даты загрузки карты.',
		'hint2'=>'Событие произошло ДО загрузки карты.',
		'hint3'=>'Авария! Карта загружена в контроллер и должна ходить!!!',
		'hint4'=>'Ходить нельзя, карта не загружена. Все верно.',
		'hint5'=>'Авария! Карта должна проходить, но не загружена в контроллер.',
		...
		'card_status_is_active'=>'Идентификатор активен',
		'card_status_status_is_not_active'=>'Идентификатор не активен',
		'about_pep_authmode'=>'Режим авторизации пользователя',
		'analit_error' => '<b>Нарушения режима СКУД</b>',
		'650a' => 'Отказ в проходе правильный. Причина: пользователь не активен, карта не активна, разрешения на проход нет.',
		'651a' => 'Отказ в проходе правильный. Причина: пользователь не активен, карта не активна.',
		'652a' => 'Отказ в проходе правильный. Причина: пользователь не активен, разрешения на проход нет.',
		'653a' => 'Отказ в проходе правильный. Причина: пользователь не активен.',
		'654a' => 'Отказ в проходе правильный. Причина: карта не активна, разрешения на проход нет.',
		'655a' => 'Отказ в проходе правильный. Причина: не активна карта.',
		'656a' => 'Отказ в проходе правильный. Причина: разрешения на проход нет.',
		'657a' => 'Отказ в проходе НЕ правильный. Имеются все разрешения, карта в контроллер загружена. См. "Коды аналитики и их описание" для кода ',
		'658a' => 'Отказ в проходе НЕ правильный (переходное состояние). Причина: Надо дождаться загрузки.',
		'500a' => 'Проход ошибочный: пользователь не активный, карта не активна, разрешения на прохода нет.',
		'501a' => 'Проход ошибочный: пользователь не активный, карта не активна.',
		'502a' => 'Проход ошибочный: пользователь не активный, разрешения на прохода нет.',
		'503a' => 'Проход ошибочный: пользователь не активный.',
		'504a' => 'Проход ошибочный: карта не активна, разрешения на прохода нет.',
		'505a' => 'Проход ошибочный: карта не активна.',
		'506a' => 'Проход ошибочный:  разрешения на прохода нет.',
		'507a' => 'Проход правильный: пользователь активный, карта активна, разрешения на прохода есть.',
		'508a' => 'Ошибка СКУД: Пользователь активный, карта активна, разрешение на проход есть, но карта стоит на удаление.',
		'509a' => 'Проход разрешен правильно, работа в режиме Единый список.',
		'-1a' => 'Событие не попадает по список анализируемых',
		'5001a' => 'Пользователь и карты не активны, проход запрещен. Карта стоит в очереди на удаление.',
		'5011a' => 'Пользователь и карты не активны. Карта стоит в очереди на удаление.',
		'5021a' => 'Пользователь не активен, да и проход запрещен. Карта стоит в очереди на удаление.',
		'5031a' => 'Пользователь не активен. Карта стоит в очереди на удаление.',
		'5041a' => 'Карта не активна, прохода запрещен. Карта стоит в очереди на удаление.',
		'5051a' => 'Карта не активна. Карта стоит в очереди на удаление.',
		'5061a' => 'Проход запрещен. Карта стоит в очереди на удаление.',
		'80a' => 'Проход неизвестного идентификатора.',
		'145a' => 'Проход в режиме ТЕСТ.',
		'480a' => 'Отказ в повторном проходе правильный.',
		'481a' => 'Отказ в повторном проходе НЕ правильный.',
		...
		'guid_506'	=> 'Причина ошибки: код карты не удалился из памяти контроллера и карта продолжает "ходить".<br> Такая ситуация возможна после ручного вмешательства в очередь загрузок либо после замены контроллера.<br>Рекомендации: Необходимо  удалить карты из контроллеров. Это можно сделать выполнив последовательно команды "Очистить" и "Записать карты" для выбранной точки прохода.',
		...
		'guid_not_active_device_panel_desc' => 'Указанные ниже точки прохода отмечены как неактивные и карты в них не загружаются.<br>Рекомендую удалить эти точки прохода из категорий доступа в программе Конфигуратор в разделе Категории доступа.<br>НЕ удаляйте точки прохода в контроллерах в разделе Устройства.',
		...
		'guid_657'=>'Проверьте конфигурацию контроллера: она должна соответствать настройкам контроллера в базе данных (используйте программу Конфигуратор, опцию "Единый список")',
		'guid_508'=>'Карты в очередь на удаление не должно быть. Рекомендуемые действия:<br>1. На странице Очередь загрузки убедитесь, что карта действительно стоит в очереди на удаление. Возможно, что очередь уже обработана, и ошибки нет.<br>2. Сделать карту пользователя неактивной, а затем опять активной. Это вызовет процесс удаления и последующей загрузки карты в контроллеры, и очередь на удаление очиститься.',
		...
		'checkConfig2'=>'Неправильная настройка оборудования: в БД СКУД режим "Единого списка" вЫключен, а в контроллере вКлючен. Снимите метку "Единый список" в контроллере и загрузите карты в контроллер. (checkConfig)',
		'checkConfig1'=>'Установите метку "Единый список" в контроллере. (checkConfig)',
		'checkConfig0'=>'Необходимо записать карту в этот контроллер. Проверьте, что количество карт по базе данных не превышает емкость контроллера. Режим ТЕСТ выключен, настройки совпадают.  (checkConfig)',
		'checkConfig3'=>'Необходимо записать карту в этот контроллер. Проверьте, что количество карт по базе данных не превышает емкость контроллера. (checkConfig)',
		'checkConfig4'=>'Включен режим "ТЕСТ". Настройки совпадают. Анализ затруднен. В этом режиме контроллер пропускает любую карту и всегда формирует событие вида "Недействительная карта". (checkConfig)',
		'checkConfig7'=>'Включен режим "ТЕСТ". Настройки совпадают. Анализ затруднен. В этом режиме контроллер пропускает любую карту и всегда формирует событие вида "Недействительная карта". (checkConfig)',
		...
		'standalone' => 'Он-лайн, загрузка идентификаторов не требуется',
		'device_data_desc' => 'Параметры контроллера',
		'id_dev' => 'id_dev контроллера',
		'id_devtype' => 'Тип контроллера',
		'devtype_edit2' => 'Сохранить изменения',
		'cardidx_refresh_log'=>'Обновление таблицы cardidx_refresh для точки прохода  = id_dev выполнено за :during секунд.',
		
		'code is 1'=>'Память контроллера заполнена полностью, запись карт невозможна',
		'recv()'=>'Нет связи с контроллером.',
		'not found'=>'Проверьте настройки транспортного уровня с помощью программы TS2Cfg. Контроллер отсутвует в настройках, либо работа с ним запрещена.',
